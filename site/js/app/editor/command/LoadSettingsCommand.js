  ...
  function ($, Config) {
    ...
    LoadSettingsCommand.prototype.execute = function (editor) {
      ...
      if (storage) { // check if browser is compatible with HTML5 Local Storage
        ...
        // remote server settings value
        var host = storage.getItem(Config.LS_REMOTE_SERVER_HOST) || Config.REMOTE_HOST;
        var port = storage.getItem(Config.LS_REMOTE_SERVER_PORT) || Config.REMOTE_PORT;
        $('#remote-server-host').val(host);
        $('#remote-server-port').val(port);
