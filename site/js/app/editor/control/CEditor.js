define(
    ...
    function (Pooffs, KEditor, KGroup, KComponent, KChannel, KNode, AController, UIEditor) {
        ...
        CEditor.prototype.p2cEntityDraggedOver = function (entityType, tDefName, tDefVersion) {
            if (!this._draggedEntity) {
                var cFactory = require('factory/CFactory').getInstance();

                switch (entityType) {
                    case KGroup.ENTITY_TYPE:
                        this._draggedEntity = cFactory.newGroup(this, tDefName, tDefVersion);
                        break;

                    case KChannel.ENTITY_TYPE:
                        this._draggedEntity = cFactory.newChannel(this, tDefName, tDefVersion);
                        break;

                    case KNode.ENTITY_TYPE:
                        this._draggedEntity = cFactory.newNode(this, tDefName, tDefVersion);
                        break;

                    case KComponent.ENTITY_TYPE:
                        this._draggedEntity = cFactory.newComponent(this, tDefName, tDefVersion);
                        break;

                    default:
                        console.error("CEditor.p2cEntityDraggedOver(libItem, entity_type, name): I don't know this entity type: "+entityType);
                        return;
                }
            }
        }
