...
angular.module('editorApp').run(function() {
	var STATEMENTS = [
		'add',
		'attach',
		'bind',
	...
	function createElement(data) {
		return {
			...
			render: function(elem) {
				elem = angular.element(elem).append(angular.element('<span class="type ' + data.type + '"></span>').append(data.type.substr(0, 1))).append(angular.element('<span class="text"></span>').append(data.text));
				if (data.desc) {
	...
	function createElemData(type, desc) {
		return function(elem) {
			return {type: type, text: elem.name, desc: desc};
		};
	}
	...
	function getInstances(instance) {
		if (instance.metaClassName() === 'org.kevoree.ContainerRoot') {
			return instance.nodes.array.map(createElemData('node')).concat(instance.groups.array.map(createElemData('group'))).concat(instance.hubs.array.map(createElemData('chan')));
		}
		return [];
	}
	...
	function getBindings(instance) {
		if (instance.metaClassName() === 'org.kevoree.ComponentInstance') {
			return instance.provided.array.map(createElemData('input')).concat(instance.required.array.map(createElemData('output')));
		}
		return [];
	}
	...
	CodeMirror.registerHelper('hint', 'kevscript', function(cm) {
		...
		if (match) {
			...
			var path = token.string.split('.').filter(function(str) {
			...
			switch (statement) {
				case 'set':
					...
					if (instance) {
						list = getComponents(instance).concat(getInstances(instance)).concat(getAttributes(instance));
					}
					break;
				...
				case 'unbind':
					instance = getInstance(path, cm.options.lint.lintedModel);
					if (instance) {
						list = getComponents(instance).concat(getInstances(instance)).concat(getBindings(instance)).filter(function(elem) {
							if (path.length === 0) {
				...
				case 'network':
					if (path.length === 0) {
						list = cm.options.lint.lintedModel.nodes.array.map(createElemData('node'));
					} else if (path.length === 1) {
						list = cm.options.lint.lintedModel.findNodesByID(path[0]).networkInformation.array.map(createElemData('network'));
					} else if (path.length === 2) {
						list = cm.options.lint.lintedModel.findNodesByID(path[0]).findNetworkInformationByID(path[1]).values.array.map(createElemData('value'));
					}
					break;
				...
				case 'detach':
					if (path.length === 0) {
						list = cm.options.lint.lintedModel.nodes.array.map(createElemData('node')).concat(cm.options.lint.lintedModel.groups.array.map(createElemData('group')));
					}
					break;
			}
		} else {
			...
			if (token.type !== 'comment') {
				...
				list = STATEMENTS.map(function(stat) {
					return {type: 'statement', text: stat};
				});
			}
		}
