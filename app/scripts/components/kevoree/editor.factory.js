            ...
            switch (trace.elementAttributeName) {
              ...
              case 'value':
                ...
                if (trace.source.getRefInParent() === 'metaData' && trace.source.name === KWE_POSITION) {
                  ...
                  if (trace.source.eContainer()
                    .getRefInParent() === 'nodes') {
                    processRefreshRecursively(trace.source.eContainer());
                  } else if (trace.source.eContainer()
                    .getRefInParent() === 'groups') {
                    trace.source.eContainer()
                      .subNodes.array.forEach(function(node) {
                        if (ui.editor) {
                    ...
                    .getRefInParent() === 'hubs') {
                    trace.source.eContainer()
                      .bindings.array.forEach(function(binding) {
                        if (ui.editor) {
                          ui.createBinding(binding);
                        }
                      });
                  ...
                  .getRefInParent() === 'dictionary') {
                  if (ui.editor) {
                    ui.updateValidity(trace.source.eContainer()
                      .eContainer());
                  }
    ...
    KevoreeEditor.prototype = {
      ...
      invokeModelUpdateListeners: function(event) {
        if (this.modelUpdateListenersEnabled) {
          Object.keys(this.modelUpdateListeners)
            .forEach(function(id) {
      ...
      invokeNewModelListeners: function() {
        Object.keys(this.newModelListeners)
          .forEach(function(id) {
      ...
      drawModel: function() {
        if (ui.editor) {
          ...
          ui.order();
          this.model.select('**/metaData[name=' + KWE_FOLDED + ']')
