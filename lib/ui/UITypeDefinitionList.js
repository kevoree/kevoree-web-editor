    ...
    update: function () {
        ...
        while (tDefs.hasNext()) {
            ...
            while (libs.hasNext()) {
                lib = libs.next();
                if (lib.name !== 'Default') {
                    var tdef = lib.findSubTypesByID('name='+tDef.name+',version='+tDef.version);
                    if (tdef) {
        ...
        while (libraries.hasNext()) {
            ...
            tDefs = lib.subTypes.iterator();
            while (tDefs.hasNext()) {
                tDef = tDefs.next();
                if (!addedTDefs[tDef.name]) {
                    $('.type-definition-list-group[data-library="'+lib.name+'"]').append(EditorTemplates['typedef-item'].render({
                        name: tDef.name,
        ...
        domTDefItems.on('dblclick', function (e) {
            ...
            var tDef = this.editor.getModel().findTypeDefinitionsByID('name='+tDefName+',version='+latestVersion);
        ...
        $('#editor').droppable({
            ...
            drop: function (event, ui) {
                var domTdef = ui.draggable.find('.typedef-name'),
                    tDefName = domTdef.html(),
                    latestVersion = ModelHelper.findLatestVersion(tDefName, this.editor.getModel()),
                    tDef = this.editor.getModel().findTypeDefinitionsByID('name='+tDefName+',version='+latestVersion),
                    metadata =  {x: event.pageX, y: event.pageY};

                if (domTdef[0].dataset && domTdef[0].dataset.tdef === 'node') {
                    var editorOffset = $('#editor').offset();
