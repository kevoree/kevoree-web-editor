...
var UITypeDefinitionList = Class({
    ...
    update: function () {
        ...
        if (!defaultLib) {
            var defaultLib = this.addLibraryCmd.execute('Default');
            ...
            while (tDefs.hasNext()) {
                ...
                var hasParentLib;
                while (libs.hasNext()) {
                    var tdef = libs.next().findSubTypesByID(tDef.name+'/'+tDef.version);
                    if (tdef) {
        ...
        $('#editor').droppable({
            accept: function (ui) {
                if (window.innerWidth >= 768) {
                    if (window.event.pageX > this.domTDefList.width()) return true;
                    return false;
                } else {
                    if (window.event.pageY > this.domTDefList.height()) return true;
                    return false;
                }
            }.bind(this),
