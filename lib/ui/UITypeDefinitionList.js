    ...
    update: function () {
        ...
        $('.typedef-name').each(function (index, item) {
            ...
            if ($(item).text().toLowerCase().search(this.keyword) !== -1) {
                switch (type) {
                    case 'node':
                        if (this.nodeState) $(item).parent().show(); 
                        else $(item).parent().hide();
                        break;
                    
                    case 'channel':
                        if (this.chanState) $(item).parent().show();
                        else $(item).parent().hide();
                        break;
                    
                    case 'group':
                        if (this.groupState) $(item).parent().show();
                        else $(item).parent().hide();
                        break;
                    
                    case 'component':
                        if (this.compState) $(item).parent().show();
                        else $(item).parent().hide();
                        break;
                }
            } else  {
