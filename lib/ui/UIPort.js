...
var UIPort = AbstractEntity.extend({
    ...
    construct: function (instance, editor, isProvided) {
        ...
        this.textBg.offsetX(this.text.getWidth()/2);
        this.textBg.offsetY((this.text.getHeight()/2) - 20);

        var timeout;
        this.text.on('mouseover', function () {
            timeout = setTimeout(function () {
                this.text.setText(instance.portTypeRef.name);
                this.textBg.setWidth(this.text.getWidth());
    ...
    update: function () {
        if (this.shape.getLayer()) {
            this.textBg.fill((this.instance.eContainer().started) ? '#000' : '#333');
        }
    },
