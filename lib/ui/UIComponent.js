...
var UIComponent = HostedEntity.extend({
    ...
    construct: function (instance, editor) {
        ...
        this.text = new Kinetic.Text({
            text:       instance.name + '\n' + ((instance.typeDefinition) ? instance.typeDefinition.name : '???'),
            fontSize:   14,
        ...
        function addPortInstances(portTypeList, isProvided) {
            function alreadyCreated() {
                ...
                while (portList.hasNext()) {
                    if (portList.next().portTypeRef === portTypeRef) return true;
                }
                return false;
            }
            ...
            while (portTypeList.hasNext()) {
                ...
                if (!alreadyCreated()) addPortCmd.execute(instance, portTypeRef, isProvided);
            }
        }
        ...
        if (providedCount > 1 || providedCount > 1) {
            if (providedCount >= requiredCount) {
                this.setHeight(providedCount*(portHeight+PORTS_PADDING)+PORTS_PADDING);
            } else {
                this.setHeight(requiredCount*(portHeight+PORTS_PADDING)+PORTS_PADDING);
            }
        }
        ...
        this.shape.on('dragstart', function (e) {
            ...
            console.log('dragstart', instance.name, 'in', this.nodeHost.name);
    ...
    onSet: function (sourcePath, attrName, value) {
        console.log('UIComponent', sourcePath, attrName, value);
        this.update();
