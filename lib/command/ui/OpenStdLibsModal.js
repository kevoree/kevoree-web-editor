...
var OpenStdLibsModal = AbstractCommand.extend({
    ...
    execute: function () {
        ...
        function renderList(platform, libraries) {
            ...
            $('.std-lib-item').off('click').on('click', function (e) {
        ...
        var modal = $('#modal');
        modal.one('shown.bs.modal', function loadContent() {
            modal.one('show.bs.modal', loadContent.bind(this)); // reload content when re-displaying modal (using shortcut)
            ...
            LIBS.forEach(function (platform) {
                tasks.push(function (cb) {
                    this.remoteLoadCmd.execute(platform, function (err, libs, model) {
                        if (err) {
