    ...
    execute: function () {
            ...
            (function (platform) {
                ...
                this.remoteLoadCmd.execute(platform.toLowerCase(), function (err, libs) {
                    ...
                    setTimeout(function () {
                            ...
                            for (var i in loadedLibs[platform]) {
                                for (var j in loadedLibs[platform][i].versions) {
                                    libVers.push({
                                        ...
                                        show:       (function () {
                                                        if ($('#filter-std-libs-release').hasClass('active')) {
                                                            return (!loadedLibs[platform][i].latest.contains('SNAPSHOT') && loadedLibs[platform][i].versions[j] === loadedLibs[platform][i].latest);
                                                        } else if ($('#filter-std-libs-snapshots').hasClass('active')) {
                                                            return (loadedLibs[platform][i].versions[j].contains('SNAPSHOT'));
                                                        }
                                                        return true;
                                                    })()
                                    });
                        ...
                        $('#filter-std-libs-release').off('click');
                        $('#filter-std-libs-release').on('click', function () {
                            if (!$(this).hasClass('active')) {
                                $(this).addClass('active');
                                $('#filter-std-libs-all').removeClass('active');
                                $('#filter-std-libs-snapshots').removeClass('active');
                        ...
                        $('#filter-std-libs-all').on('click', function () {
                            if (!$(this).hasClass('active')) {
                                $(this).addClass('active');
                                $('#filter-std-libs-release').removeClass('active');
                                $('#filter-std-libs-snapshots').removeClass('active');
                                $('.std-platform-tab').each(function () {
                                    displayList($(this).text());
                                });
                        ...
                        $('#filter-std-libs-snapshots').off('click');
                        $('#filter-std-libs-snapshots').on('click', function () {
                            if (!$(this).hasClass('active')) {
                                $(this).addClass('active');
                                $('#filter-std-libs-all').removeClass('active');
                                $('#filter-std-libs-release').removeClass('active');
        ...
        modalSaveBtn.on('click', function () {
            ...
            $('.std-lib-item input:checked').each(function () {
                var platform = this.parentElement.parentElement.dataset.platform;
                libsToMerge[platform] = libsToMerge[platform] || [];
                libsToMerge[platform].push({
